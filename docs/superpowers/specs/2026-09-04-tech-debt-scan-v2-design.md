...
**Phase 5b: the measurement** (`feat/tech-debt-scan-v2-phase-5b`). Scope, in order: repair the corpus defects that contaminate every row of `docs/evaluation-log.md` — `web-ts`'s decoy `d1` gains the `npm ci` its workflow was always meant to have, every planted decoy gains an explicit `sources` list that `evaluate.hits` honours so a true finding can no longer be scored against a decoy planted for a different scout, and one fixture gains a genuinely zero-churn file so the dead-code cap of 2.3 can award tier B and the family's recall is measurable at all; the hotspot-correlation test then runs on all three fixtures — it already runs on `service-py` and, since 4b's clone pair, on `web-ts`; `mixed-decoys` still skips because fewer than four of its planted paths score as source files; the two 4b carry-forwards that change what is measured land before the run — the hadolint range guard reordered so a fact with an unusable range can still merge into a rule finding, and gitleaks' `StartColumn` kept so two secrets on one line stay two candidates; the note agent joins `live_run.py`, so the design document's remediation sections come from a model for the first time; then one live run over the three repaired fixtures, same model and settings as every prior row.

**Setting the bar.** The 0.80 figure was provisional from the first line of this document and thirteen live runs have placed the real number between 0.25 and 0.73, with decoys reaching tier A in three of the most recent six — some of them fixture defects rather than tool failures. 5b replaces it with a measured bar: the **minimum** tier A precision across the three repaired fixtures in 5b's run, rounded down to the nearest 0.05, recorded in section 1 and section 6 as *measured on the date of that run*. Zero decoys at tier A or in the top N stays a hard requirement, because the `sources` list makes a decoy hit unambiguous. Raising the bar later requires a run that clears it. Gate: the corpus repairs pinned by tests; the note agent's output rendering through the notes contract; the live run's rows in the log; the bar written into the spec from those rows and not from hope.
