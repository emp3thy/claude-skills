...
2. On the fixture corpus no planted decoy reaches tier A. Tier A precision under the opt-in live run is measured against a bar that phase 5b sets from a run over the repaired corpus — the minimum across the three fixtures, rounded down to 0.05 — replacing the provisional 0.80 this document carried until then; 5b's run of 2026-09-09 set none, because a decoy (`d2`) reached tier A on web-ts, so the provisional 0.80 stands until a run clears the hard gate. `docs/evaluation-log.md` records every run against it.
...
| `tools_probe.py`, `tool_normalisers.py`, `plan_scan.py`, `merge_findings.py`, `verify_prompts.py`, `apply_verdicts.py`, `rank.py`, `baseline.py`, `evaluate.py` | new (4.5 to 4.10, section 6) |
...
### 4.5 `tools_probe.py` and `tool_normalisers.py`
...
**First-cut tools (10):** osv-scanner (any lockfile; vulnerabilities for TD-02 and TD-03), gitleaks (secrets), ruff (Python: E722, BLE001, S110, S112 for error masking; C901, PLR091x for complex units; F401 for dead imports; UP035 for deprecated imports; its `filename` is absolute with backslashes even for relative input and is relativised and forward-slashed; it reads the repository's own ruff configuration unless run `--isolated`), vulture (Python dead code with per-kind confidence), lizard (per-function NLOC, CCN and parameter count), jscpd (clones), knip (JS and TS dead exports), madge (JS and TS cycles), hadolint (Dockerfiles), actionlint (workflows). Later cuts once a normaliser has goldens: dependency-cruiser, import-linter, pip-audit, npm outdated, semgrep, bandit, trivy, zizmor, checkov, kube-linter, `dotnet list package`, Go deadcode and govulncheck. Only ruff, vulture, lizard, jscpd, knip and madge are installed on the development machine; osv-scanner, gitleaks, hadolint and actionlint have no normaliser written against a real payload, so their fixtures are canned. The corpus goldens are live runs of whatever is installed, not canned output: each golden records `covered_tools` -- the tools whose status, reason and signals it pins -- and the comparison is that set intersected with what the running machine can exercise, so a machine with a different toolchain neither fails the suite nor silently compares nothing.
...
**Secrets in tool output.** gitleaks reports the credential it matched, in `Secret` and `Match`. The normaliser drops both fields outright rather than redacting them: `tool-signals.json` is written to the workdir and read into prompts, and a redacted secret is still the secret's first four characters while a dropped one is nothing. What is kept — rule id, file, line range and entropy — is all the verifier needs to judge whether a hit is a fixture or a placeholder, and all a human needs to find it. Any later normaliser whose tool reports matched values follows the same rule.
...
Reads the workdir and config, decides scope and chunking, renders every prompt to `prompts/scout-<family>[-<module>].md`, and writes `scan-plan.json`. SKILL.md dispatches exactly the entries in the plan. An absent `tool-signals.json` (phase 3, and any run with the probe skipped) is treated as every tool `skipped`. In phase 2 every set form (`default`, `quick`, `deep`, an explicit list) is accepted, `chunked` is always false and the thresholds are recorded only; reading `tool-signals.json`, module chunking and the halved deep thresholds arrive in phase 4b.
...
{ "schema_version": 2,
  "candidates": [ { "fingerprint": "", "quote_hash": "", "family": "", "debt_type": "", "type_id": null, "title": "",
                    "severity": 3, "effort": "M", "source": "scout|rule|tool", "rule_id": null, "tool": null, "note": "",
                    "evidence": [ {"file": "", "line_start": 0, "line_end": 0, "column": null, "quote": "", "quote_verified": true} ],
...
Rule findings and osv facts carry `tier: "A"` already; every other candidate has `tier: null` until `apply_verdicts.py`. Rule findings are appended unchanged after the scout candidates and corroborate rather than merge. `missing_file` and `dropped_reasons` appear only when they apply. The `column` key is present on fact-class tool evidence only (gitleaks' `StartColumn`); a scout item is rebuilt by `_clean_evidence` without it.
...
**The fact-class bypass.** Assigning a tier without a verdict is a single, named path: a candidate whose `source` is `rule`, or whose `source` is `tool` and whose signal is fact-class and non-verifiable (osv at manifest level), is tiered directly and never enters the verdict join. Every other candidate reaches its tier through a verdict or through `unverified`. This is the only route by which a finding reaches tier A without a verifier having read it, so phase 4b's gate asserts both halves: that the bypass works, and that no other candidate class takes it. gitleaks is deliberately not on this path — placeholders and test fixtures produce false positives that only a verifier can dismiss.
...
`baseline.py diff` compares `verified.json` against the committed baseline and writes `diff.json`; `baseline.py record` (called in-process by `promote.py`) writes decisions back, and also writes a `pending` entry for every verified finding that has no decision and no entry, and on an entry it saw again refreshes the finding's location and `last_seen` while keeping its status, reason, until, bundle and first_seen; an entry whose finding it matches only by the edited heuristic migrates to the new fingerprint with its decision, and the old key is removed, so a finding seen and undecided is UNCHANGED on the next scan rather than NEW and a suppression survives an edit. Both are subparsers; `--baseline` defaults to the config `baseline` key, `.tech-debt/baseline.json`.
...
2. `# Top N`, then one H2 per finding with the anchor `status`, `slug`, `fingerprint`, `tier`, `priority`, `family`, `category`, `debt_type`, `type_id`, `severity`, `effort`, `diff`; sections `### Proof` (verifier text), `### Evidence` (one line per item as `path:start-end` followed by the quote in a fenced block), `### Signals` (hotspot score, churn, coupling pairs, fan-in labelled approximate, `confirmed_by`), `### Remediation` and `### Acceptance criteria` (from the note agent). Both note sections are rendered for every top-N finding whether or not a note exists, carrying the placeholder `remediation note not available` when it does not, so a finding's section shape never depends on the agent having answered. `category` is always emitted as the alias of `family`: it is a required parser key and `bundle_writer.py` reads it unconditionally.
3. `# Below the cut`: compact H2 sections for every remaining tier A and B finding (anchor plus Proof and Evidence only), so they are promotable. The table of tier C and unverified candidates (slug, family, file, reason) then follows under its own H1, `# Below the cut: tier C and unverified`, immediately after those compact sections: a table placed after the last H2 of a section is inside that finding's body under the parser's own rules, and would be copied into that finding's PBI. Reading order is unchanged; the parser boundary is explicit.
...
**`findings.json`** holds every candidate that reached `verified.json`, with `fingerprint`, `slug`, `title`, `family`, `debt_type`, `type_id`, `severity`, `effort`, `evidence`, `signals`, `confirmed_by`, `tier`, `verdict`, `proof`, `priority`, `terms`, `in_top_n`, `spread_capped`, `diff` and, from 5b, the producer fields `source`, `rule_id` and `tool` copied from the candidate. It is the machine-readable twin of `design.md` and the input to `evaluate.py`, which needs the producer fields to honour a decoy's `sources` list.
...
**Flags:** `/tech-debt-scan <repo> [--quick | --deep] [--preset balanced|hotspot-first|architecture|quick-wins] [--families a,b,c] [--top N] [--no-tools]`. `--quick` selects the quick set and `--top 3`; `--deep` selects the deep set; `--families` overrides both and bypasses the adaptive rule; halving the chunking thresholds is a consequence of the selected set being `deep`, not of the flag, so `--deep` and `--families deep` cannot behave differently; `--no-tools` runs step 4 with `--skip-all`.
...
**Scan steps.** Step numbers are fixed across phases: phase 3 ships the list without steps 4 and 11, which phases 4b and 5 insert without renumbering.
...
Three fixtures: `service-py` (Python: a hotspot, a coupled pair, a knowledge island, an empty catch, an untested module, a two-year-old FIXME, a Dockerfile without USER, a workflow without timeout, and a seeded true-positive credential in a non-test source file so the credential rule and the redaction path have a positive case), `web-ts` (TypeScript: a three-file cycle, a co-committed near-duplicate pair, a deprecated helper still called, a permanently-off flag) and `mixed-decoys` (written in one of C#, Go or Java: a 300-line lookup table, a manifest entry point with no caller, a `getattr`-style string dispatch, a fluent builder, a `main()` that catches, logs and exits non-zero, a documented kill-switch flag, a dev-only compose file with `latest`). **Multi-language requirement:** the three fixtures cover Python, TypeScript and a third language family, and every pattern rule has a positive case in at least two of them; no rule is validated on one language alone. **Dead-code limit, and its repair in 5b:** until phase 5b every file in all three fixtures was committed inside the scoring window, so none had zero churn and the 2.3 dead-code cap (tier C unless churn and fan-in are both zero) could never award tier B to a planted dead-code item; dead-code recall read 0.00 on this corpus by construction. 5b gives one fixture a genuinely zero-churn file so the cap is exercised and the family measurable: `service-py`'s planted dead-code file `src/pay/legacy_export.py` (p14 and p15, both `expect_tier: B`) moves out of the fixture's first commit into its own commit dated 2004-01-01, twenty-two years before the 240-month window, and no later commit touches it, so its churn is 0 and its fan-in 0; goldens carrying churn and hotspot figures move with it, and the canned scout replies still cite a file that exists. **Decoy sources, from 5b:** each decoy carries a `sources` list of producer tokens in the vocabulary the chain already uses — `scout:<family>`, `rule:<rule_id>`, `tool:<name>`. A finding's producers are its own token — `scout:<family>` when `source` is `scout`, `rule:<rule_id>` when it is `rule`, `tool:<tool>` when it is `tool`, read from the `source`, `rule_id` and `tool` fields the candidate carries (4.7) and `findings.json` copies (4.11) — together with every `confirmed_by` token; a family mismatch still rules a hit out first, so the list only ever distinguishes producers within the decoy's own family; `evaluate.hits` scores a finding against a decoy only when the two sets intersect, so a true finding from another producer landing on a decoy's path is not counted as a false positive. A decoy with no `sources` list matches any producer, as before 5b, and a corpus test asserts that every decoy in the three fixtures carries a non-empty list, so that fallback never runs on the corpus. `web-ts`'s `d1` is the case that motivated it: the workflow runs `npm test` under a retry action without ever installing dependencies, no CI rule fires on it, and the pipeline-infra scout's report of the missing install was a true finding scored against a decoy planted to prove that a well-configured workflow draws no finding; 5b gives the workflow a `run: npm ci` step before the retry action, so the decoy is a decoy again, and a corpus test pins the step's presence. **The hotspot-correlation test, from 5b:** it compares planted files against source files that are neither planted nor decoys; `mixed-decoys` has ten source files, five planted and five decoys, so that comparison set was empty and the test skipped. 5b adds two neutral Go files to `mixed-decoys` — a couple of commits each, trivial complexity — so the test runs on all three fixtures. The v1 fixtures stay for the inventory count tests.
...
**Live policy.** `scripts/live_run.py <fixture>` (phase 2) replays a fixture, runs the signal scripts and `plan_scan.py`, dispatches every scout and verifier batch through `claude -p` (print mode, JSON output, read-only tools `Read`, `Grep`, `Glob`, user settings and MCP servers excluded with `--setting-sources project --strict-mcp-config --disable-slash-commands`, a per-call `--max-budget-usd`), then runs `merge_findings.py`, `verify_prompts.py`, `apply_verdicts.py` and `rank.py`; from 5b it then renders `prompts/notes.md`, dispatches the single note agent through the same `claude -p` isolation, model and budget, validates the reply against a `NOTES_SCHEMA` derived from the notes contract (a list of `fingerprint`, `remediation`, `acceptance_criteria`) and writes `notes.json`; renders `design.md` and `findings.json` through `design_writer.write_design` with its parser self-check; and only then runs `evaluate.py`, which reads `findings.json`. The harness never runs `baseline.py diff`; because `design_writer` drops baseline-suppressed findings from `findings.json`, the harness refuses to score when a `diff.json` or a baseline file is present in the workdir, so a filtered population can never set a bar. `--skip-agents` reuses an existing `notes.json` as it reuses scout and verdict files. `pytest -m live` wraps the same harness. Both are invoked manually by the maintainer, never in CI. The scout and verdict outputs of the first live run become the canned goldens, with two hand edits per fixture (one invented quote that pins the diversion path, one trap case). It appends a dated row (tier A precision, per-family precision and recall, decoy hits, token totals, and from 5b a trailing `notes` column giving the top-N findings with a real remediation note over N) to `docs/evaluation-log.md`; the column is appended last so no historic cell moves. From 5b each run's `evaluation.json`, `design.md`, `notes.json` and `findings.json` are kept under `docs/research/tech-debt-scan-v2/runs/<date>-<phase>/<fixture>/`, redacted at write as every document is, so a bar can be audited against the documents it came from. Release bars: zero decoys in tier A or the top N is hard from v2.0; tier A precision is reported against the bar phase 5b sets from the repaired corpus, which replaces the provisional 0.80 — 5b's run of 2026-09-09 set none, because a decoy reached tier A on web-ts, so the provisional 0.80 stands until a run clears the hard gate; recall is reported without a bar in v2.0. The measured false-positive rate replaces the 50 to 63 percent scout prior quoted in the research.
...
| Verifier and tiers | 60% | bounded context is nearer the 36.4 percent condition than the 95.5 percent one; the 0.80 bar is unsupported | bounded exploration allowance, corpus with decoys, per-family questions, traps from rejections, bar reported at v2.0 and hard at v2.1; 5b's run measured 0.28 to 0.57 with a decoy at tier A on web-ts, so the bar stays provisional |
...
**Phase 5a: the baseline** (`feat/tech-debt-scan-v2-phase-5a`). Scope: `baseline.py` diff and record per 4.10; `diff.json` loaded into `design_writer`'s inputs so the `diff` anchor key `_diff_for` already renders stops reading `NEW` for everything; promote write-back calling `baseline.py record` in process and returning `EXIT_WRITE_BACK` (6, reserved since phase 3) when the write-back is what failed; the gitignore triple; `accepted` expiry; step 11 and the `--baseline` promote flag in SKILL.md; and the documentation-only carry-forwards from earlier phases. Everything here is deterministic and testable without spending a token, which is why it lands before anything measures with it. Gate: the baseline transition tests of 4.10 over the synthetic history; promote write-back tests including exit 6; the triple appended once and never duplicated, with `git check-ignore` confirming the baseline is tracked; `test_real_skill_md_passes`. Afterwards re-scans show NEW and RESOLVED, rejected findings stop recurring, and accepted findings return when their expiry passes.

**Phase 5b: the measurement** (`feat/tech-debt-scan-v2-phase-5b`). Scope, in order, every step deterministic and pinned by a test until the run itself. First the corpus repairs that contaminate every row of `docs/evaluation-log.md`, as section 6 now states them: `web-ts`'s workflow gains `npm ci` so decoy `d1` is a decoy again; every one of the 27 planted decoys gains a `sources` list that `evaluate.hits` honours; `service-py`'s `legacy_export.py` becomes the zero-churn file so the 2.3 dead-code cap can award tier B without a tool; and `mixed-decoys` gains two neutral source files so the hotspot-correlation test runs on all three fixtures (it skipped there because every source file was planted or a decoy, not, as an earlier version of this paragraph said, because too few planted paths were source files). Then the two 4b carry-forwards that change what is measured: `merge_findings` tries the merge of a hadolint or actionlint fact into a same-file rule finding before the range guard, since that merge reads no range, and applies the guard only to the untiered candidate route that builds evidence spans; and `normalise_gitleaks` keeps `StartColumn` in the signal's `extra`, with the candidate fingerprint span including the column when present, so two secrets on one line stay two candidates. Two one-line 4b leftovers ride along: `plan_scan`'s `(repository root)` chunk label becomes one no directory can equal, and the `test-quality` cap's `tier_reason` says CI data, as 2.3 does. Then the note agent joins `live_run.py` as section 6 describes — the notes call, the render, the guard against a baseline in the workdir, evaluation over `findings.json`, the `notes` column — gated by a harness test that drives canned scout, verdict and notes replies through to `design.md` and asserts no `remediation note not available` placeholder in the top N, and by the schema rejecting a reply whose note breaks the contract, with `design_writer.notes_by_fingerprint` dropping any note whose fingerprint is outside the top N before the render (4.11), a filter the harness test pins. Then one live run over the three repaired fixtures with tools present: `--model sonnet`, `--churn-months 240`, the same per-call budget and isolation as every prior row, about $11 in total.

**Setting the bar.** The 0.80 figure was provisional from the first line of this document and thirteen live runs have placed the real number between 0.25 and 0.73, with decoys reaching tier A in three of the most recent six — some of them fixture defects rather than tool failures. 5b replaces it with a measured bar: the **minimum** tier A precision across the three repaired fixtures in 5b's run, rounded down to the nearest 0.05, recorded in section 1 and section 6 as *measured on the date of that run*, with the run's three rows in the log and its documents under `docs/research/tech-debt-scan-v2/runs/`. Zero decoys at tier A or in the top N stays a hard requirement, because the `sources` list makes a decoy hit unambiguous. Two outcomes are possible and both are recorded: if no decoy reaches tier A or the top N, the minimum is the bar and the provisional 0.80 leaves this document; if one does, the run sets no bar, the decoy's `sources` list says whether the hit is a real false positive or a fixture defect, and that ruling is written into the ledger and the log before any second run is spent. Raising the bar later requires a run that clears it. 5b's run of 2026-09-09 put a decoy at tier A on web-ts (`d2`, `scout:error-masking`); the decoy's `why` reads `categories.py`'s definition as requiring a catch that drops the cause, and this catch logs it, while the verifier read the same site as a catch-everything variant that hides every failure behind a `null` the caller cannot tell from an empty body — `categories.py:171-176` names both shapes under "failures caught and hidden, so nobody learns of them", so the rubric admits both readings and which one the corpus adopts is the first decision of the next run; no bar was set. The three rows (service-py 0.57, web-ts 0.28, mixed-decoys 0.50) and the run's documents are under `docs/research/tech-debt-scan-v2/runs/2026-09-09-5b/`; the minimum they would have given, 0.25 rounded down, is recorded as measured but not adopted, and the provisional 0.80 stays. Whether `d2` stays a decoy or becomes planted debt is the first decision of the next run, which still must clear the hard gate to set or raise the bar. 5b does not tune a prompt against the corpus (that would fit the bar to the fixtures), does not spend a second run to improve a number, does not decide the v2.1 hard bar, and does not touch `data-ml`. Two gaps phase 5a's reviews found stay parked as known limitations for a follow-on: the frontmatter's `resolved: N` names findings the document never lists (a `# Resolved since last scan` section would make it inspectable), and a design document from an earlier scan promoted against a newer `verified.json` writes a decision with no location, after which no edited match can reach it. Gate: the corpus repairs pinned by tests; the note agent's output rendering through the notes contract; the live run's rows in the log; the bar written into the spec from those rows and not from hope.
