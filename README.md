...
parsing, validation, evidence-document rendering — is a deterministic
...
3. **Promote** — open a design session on one approved finding:

   ```
   /tech-debt-promote
   ```

   This lists the `approved` findings, lets you pick one, writes
   `.tech-debt/evidence.md` for it and flips it to `promoted` (so a re-run is
   a no-op), then hands the evidence document to `superpowers:brainstorming`,
   which always ends by invoking `superpowers:writing-plans`. One finding per
   invocation — a second finding is a second run.
...
| `.tech-debt/baseline.json` (or wherever `--baseline`/the config `baseline` key points) | `baseline.py record --design <design.md> [--workdir DIR] [--root <repo>] [--baseline <path>] [--today <date>]`, called in process by `promote.py --baseline` — never run standalone in the chain | `{schema_version: 2, last_scan, preset, findings{<fingerprint>: {family, file, line_start, quote_hash, quote, title, tier, status, first_seen, last_seen, reason, until}}}`; writes every `design.md` finding's decision back, atomically, keeping entries this scan did not touch; also writes a `pending` entry for every `verified.json` finding with no decision and no prior entry (ruling 25); an entry it sees again keeps only its `status`, `reason`, `until` and `first_seen` and has every other field refreshed from this scan, and an entry whose code was edited since migrates to the finding's new fingerprint with its decision intact, the old key removed (ruling 26); a decision with no fingerprint or an unrecognised status raises; then `ensure_gitignore_triple` appends `!<dir>/`, `<dir>/*`, `!<dir>/<name>` to the repository's `.gitignore` the first time git reports the baseline path ignored, returning `appended`, `present`, `no-git` or `still-ignored` (the triple was appended but an ancestor directory is itself wholly ignored, so the baseline stays ignored until the user un-ignores that ancestor by hand) |
...
| `evidence.md` | `promote.py --select <slug>` (`evidence_doc.render_evidence`) | the seed for a design session, written beside `design.md` and overwritten on every `--select`: a title, a `Repository`/`Scan`/`Finding`/`Tier` metadata line, then the selected finding's own body_md verbatim (`### Proof` through `### Acceptance criteria`), plus, only when they match one of the finding's cited files, a `### Open questions from the scan` section (`candidates.json`'s `open_questions`, nearest cited line first) and a `### Already ruled out` section (`candidates.json`'s `looks_bad_but_fine`) |
...
failure after `evidence.md` is already written exits 6; the evidence document
and the promoted status stay, and re-running `--select` on the same slug
re-renders it rather than refusing. Phase 5b — the live-run measurement
