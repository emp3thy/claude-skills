...
`/tech-debt-scan <repo>` runs the full fifteen-step chain (step 4, the tool
...
1. `inventory.py` writes `inventory.json` and `coupling.json`: churn,
   complexity, hotspots and change coupling, joined to the reference graph
   (`has_edge`, `edge_direction`, `lead_kind`).
1a. `concern_index.py` writes `concern-index.json`: per-directory aggregates
    and at most 40 recurring definition names, the concerns scout's only
    leads. Exits 2 when `coupling.json` predates the graph join.
...
| --- | --- | --- | --- |
| `inventory.py <repo> --workdir .tech-debt` | the tree, one `git log` pass, `.tech-debt.yaml` | `inventory.json`, `coupling.json` | path classes (tests, generated, vendored, docs, source) on code files and artefacts alike, artefact classes, per-file churn and authorship (authors keyed by email, bots dropped, joined against HEAD), `hotspot_score` and the `hotspot_band` (top 10 percent of source files, 5 to 50), blame line share on the band, change-coupling pairs (`shared >= 3`, `ratio >= 0.30`, bulk commits over 50 files excluded), approximate fan-in and fan-out by identifier stems over import-like lines with the mechanical ambiguity rule, import-line cycles of size 2 to 5 as leads, directory instability, test mapping across seven naming conventions, the docs and tests blocks, and the size guard that never reads a file over 2 MB or with a NUL byte in its first KB (`skipped_large` per entry, `skipped_large_files` at the top level); as of spec 2026-09-12 section 2, `_annotate_pairs` joins every change-coupling pair to the reference graph, adding `has_edge` (a direct edge either direction, never transitive), `edge_direction` (`a->b`, `b->a`, `both` or `null`) and `lead_kind` — `modularity-violation` when the pair co-changes with no edge between the two files, `unstable-interface` when an edge lands on a file whose fan-in sits in the top decile by count (`_fan_in_decile_floor`, `None` when the fan-in list is empty or every value is identical), `null` otherwise — and `coupling.json` gains a top-level `edges` list of the graph's own `[src, dst]` pairs |
| `concern_index.py <repo> --workdir .tech-debt` | `inventory.json`, `coupling.json` | `concern-index.json` | spec 2026-09-12 section 4: the concerns scout's deterministic index in place of file leads — per-directory aggregates copied from `coupling.json`'s own directory data, and at most `MAX_CANDIDATES` (40) definition names that recur across at least two source files in at least two directories, extracted with a per-language regex table over definition lines, normalised so `computeTotal`, `compute_total` and `ComputeTotal` collapse to one key, and filtered by a `STOPLIST` of language idioms and per-adapter verbs; each candidate carries `name`, `tokens`, `files`, `directories`, `hotspot_touch`, `hotspot_share` and `coupled`, ranked by directory count then hotspot share; exits 2 when `coupling.json` has no `edges` key, which means it predates the graph join and the chain must be re-run from step 1 |
| `patterns.py <repo> --workdir .tech-debt [--no-blame]` | `inventory.json`, the files | `patterns.json`; fills `files[].inline_disables` | regex leads per family (half-finished stubs and skips and no-timeout calls, error-masking catches with the caught variable and carrier exclusion, dead-code commented-out runs, legacy names, deprecations and flag SDK calls, security credentials with four-character redaction, string SQL, dynamic evaluation, TLS off, weak hashes, permissive CORS and suppressions, test-quality signals, stdout writes where a logger exists), four `performance` rules from `_scan_loops` (io-in-loop, regex-in-loop, sort-in-loop, membership-in-loop) and the SATD table with blame age and ticket flags; artefacts are scoped by their artefact class but every lead and SATD entry on one reports the artefact's real `path_class`, and an artefact classed `generated` or `vendored`, or marked `skipped_large`, is not scanned |
...
`scripts/categories.py` defines sixteen language-agnostic debt families
...
| `performance` | Local loop-level smells (I/O, regex, sort or membership checks in a loop) and cardinality claims (N+1, complexity, "will not scale") |
| `concerns` | A definition name — or the concept it names — scattered or duplicated across directories with no shared owner |
...
- **Type id** (`validate_type_id`): `TD-01` to `TD-37`; checked only when present.
...
`--families deep` already selects the full sixteen-family set today
