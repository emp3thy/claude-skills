...
`diff: NEW`, `UNCHANGED`, `UNCHANGED (moved)`, `UNCHANGED (edited)` or
`RESOLVED` on every finding, plus `new` and `resolved` counts in the
frontmatter. `/tech-debt-promote --baseline <path>` then writes each
