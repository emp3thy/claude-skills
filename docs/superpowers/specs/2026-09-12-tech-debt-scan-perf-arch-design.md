...
7. The fixture corpus carries at least one planted item and one decoy per new family. The corpus half is met and measured through the deterministic goldens; the live tier-A evaluation of the two new families is deferred to the project's pending bar-setting live run (the tier-A bar has been provisional at 0.80 since phase 5b).
8. `/tech-debt-scan` over a repository with no coupled pairs, no PERF hits, no recurring names and no hotspot band produces a `design.md` identical to today's apart from the two families listed under `families_skipped` with reason `no leads`.
...
**Candidate rule.** A normalised name -- lower-cased, camel and snake split to a token tuple -- defined in at least two files across at least two directories. Excluded: a stoplist of idioms (`main`, `init`, `new`, `run`, `setup`, `teardown`, `test_*`, `__init__`, `__str__`, `__repr__`) and of the bare per-adapter verbs (`normalise`, `parse`, `load`, `render`, `handle`, `build`) themselves, matched as exact, whole normalised keys rather than as prefixes -- a name that only starts with one of them, such as `parse_csv` duplicated across directories, is scatter and stays a candidate; only a definition named exactly `parse` (or another bare verb in the list) is the per-adapter idiom the stoplist means to drop. Ranked by directory count, then hotspot share; top 40 kept. Each candidate records `name`, `files`, `directories`, `hotspot_touch` (any file in the band) and `coupled` (any two of its files form a `coupling.json` pair). Output is sorted and deterministic.
...
A `coupling.json` from an earlier scan lacks the three new pair fields and `edges`; `plan_scan.py` treats an absent `lead_kind` as `null`, and a `coupling.json` without `edges` makes `concern_index.py` exit 2 with a message naming the missing key, not the 5 an earlier draft of this section named -- every sibling script returns 2 on a bad input (`inventory.py`: 2 on a bad path), and exit 5 is `SKILL.md`'s own no-improvisation abort for a missing step output, not a script's own code (plan deviation 3) -- and the chain is re-run from step 1. A baseline written before this change reads unchanged; new families produce new fingerprints and read as `NEW`. A `design.md` from an earlier scan parses unchanged: no anchor key is added.

A `quick` scan's findings, candidates and ranking are unchanged by this branch, but its `design.md` is not byte-identical, and it should not have been claimed to be. `families_skipped` is built from `FAMILIES`, the full sixteen, not from the selected set, so a `quick` plan's `families_skipped` gains two entries -- `performance` and `concerns`, each with reason `not in set` -- rendered twice, into the frontmatter's `families_skipped` block and into the "Families not run" line of the Not assessed section. Separately, because `SCOUT_OUTPUT_CONTRACT` is shared across every family, adding `files_read` to it (plan deviation 1) means every family's rendered prompt changed, the six `quick` families included, each gaining the contract's `files_read` line; a live `quick` run cannot be byte-identical either, whatever the frontmatter does. Both are accepted, recorded consequences, not regressions: a `quick` report that names the two families it did not run is more honest than one that hides them, and suppressing them would change existing behaviour for `test-quality` and `pipeline-infra` too. A test pins the `quick` set's `families_skipped` names exactly, so the next family addition is a deliberate decision rather than a silent one.
