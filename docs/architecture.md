...
4. With `--baseline <path>`, `baseline.py record` is called in process to
   ...
   suppressed finding has none, by design) and refresh `last_seen` on one
   already there, then `ensure_gitignore_triple` tracks the baseline the
...
human's decisions from an edited `design.md` back into the baseline, also
writing a `pending` entry for every verified finding a decision never
covered and refreshing `last_seen` on one already recorded, and is
...
4. With `--baseline <path>`, `baseline.py record` is called in process to
   ...
   gets a fresh `pending` entry, and one already recorded just has
   `last_seen` refreshed, so it reads UNCHANGED rather than NEW next scan;
