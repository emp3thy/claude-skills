...
2. On the fixture corpus no planted decoy reaches tier A. Tier A precision under the opt-in live run is measured against a bar that phase 5b sets from a run over the repaired corpus — the minimum across the three fixtures, rounded down to 0.05 — replacing the provisional 0.80 this document carried until then. `docs/evaluation-log.md` records every run against it.
