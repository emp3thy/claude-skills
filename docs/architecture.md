...
4. With `--baseline <path>`, `baseline.py record` is called in process to
   ...
   with their `reason` and `until` — back into the baseline, then
   `ensure_gitignore_triple` tracks the baseline the first time git reports
   it ignored (see [The baseline](#the-baseline) below).
...
`diff.json` for `design_writer.py` to render into `design.md`'s `diff`
anchor key and its `new`/`resolved` frontmatter counts; `record` writes a
human's decisions from an edited `design.md` back into the baseline, and is
never run standalone in the chain — `promote.py` calls it in process when
given `--baseline`.

**Classification.** Every current finding gets exactly one `diff` value:

- `UNCHANGED` — its fingerprint matches a baseline entry at the same
  recorded line.
...
- `RESOLVED` — a baseline entry no current finding matched (by either route
  above), once its recorded `quote` can no longer be found in the file, or
  the file itself is gone or is now a directory. An entry with no recorded
  `quote` — every entry written before `record` started writing one
  alongside `quote_hash` (this task) — fails closed instead: it stays open
  with the note `quote unavailable` rather than resolving on the mere
  absence of a way to check, so a suppressed entry is never dropped from
  the baseline just because a scan failed to reproduce its fingerprint.
- `NEW` — none of the above; also every finding when no baseline file
  exists at all (`baseline_found: false` in `diff.json`).

Only a direct fingerprint match (the first two bullets) can carry the notes
`acceptance expired` or `until is not a date`; an edited match against an
entry whose acceptance has since expired is still `UNCHANGED (edited)` with
no note, because the edited-match branch only sets a note
(`suppressed by edited match`) when the match is currently suppressing.
...
4. With `--baseline <path>`, `baseline.py record` is called in process to
   ...
   or `approved` — back into the baseline, then `ensure_gitignore_triple`
   tracks the baseline the first time git reports it ignored. See
...
none; `4` on a bundle-write failure *after* at least one bundle was written
...
`ValueError` (including malformed JSON), or an `OSError` — after the
bundles were already emitted and `design.md` already marked: the bundles
and the status flips persist, only the baseline itself did not update.
...
promote writes decisions back into it, appending the gitignore triple where
needed — see [The baseline](#the-baseline). Phase 5b, the live harness run
that repairs the corpus and sets a measured tier A precision bar in place of
