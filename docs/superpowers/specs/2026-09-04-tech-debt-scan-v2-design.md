...
`DEFAULT_IGNORE` drops `build` and `bin` from its directory list and replaces them with a check that the directory contains no manifest. The artefact walk excludes `.tech-debt.yaml` at the root (the workdir is already ignored by whole path part).
...
- `import-lines`, the default: B references A only when A's stem appears in an import-like line of B. An import-like line matches `^\s*(import|from|using|use|require|include|#include|load|open|extern crate|package|require_relative|@import|@use)\b` or contains `require(`, `import(`, `from "` or `from '`. A line ending in an open bracket, a trailing backslash or a comma is joined with the next as a continuation.
...
{ "schema_version": 2,
  ...
  "looks_bad_but_fine": [ {"file": "", "line_start": 0, "why": ""} ],
  "stats": { "<family>": { "raw": 0, "dropped": 0, "quote_failed": 0, "clustered": 0, "suppressed": 0 } } }
```

Rule findings and osv facts carry `tier: "A"` already; every other candidate has `tier: null` until `apply_verdicts.py`.
...
**Phase 5: baseline and evaluation** (`feat/tech-debt-scan-v2-phase-5`). Scope: `baseline.py` diff and record, the `diff` anchor key rendered from `diff.json`, promote write-back and the gitignore triple, `accepted` expiry, step 11 and the `--baseline` promote flag in SKILL.md, the live harness and `docs/evaluation-log.md`, the first live run and its row. Gate: baseline transition tests, promote write-back tests including exit 6, `test_real_skill_md_passes`, the live run reporting tier A precision against the provisional 0.80 bar with zero decoys at tier A or in the top N. Afterwards re-scans show NEW and RESOLVED, rejected findings stop recurring, and accepted findings return when their expiry passes.
